...
const VictimStatusChat = ({ request, onClose }) => {
    ...
    const reqIdStr = String(request.id); // Ensure room ID is string
    ...
    const handleConflictSubmit = (reason) => {
        socket.emit('raise_conflict', {
            requestId: reqIdStr, reporterRole: userRole, reason: reason,
        });
        setIsConflictModalOpen(false); 
    };

    // Handler for Solved button
    const handleMarkSolved = () => {
        if (window.confirm("Are you sure the issue is fully resolved? Clicking YES confirms completion.")) {
            ...
            socket.emit('mark_solved', {
                requestId: reqIdStr, reporterRole: userRole 
            });
        }
    };
    ...
    return (
        <div className="form-overlay" style={{ background: 'rgba(0, 0, 0, 0.6)' }}>
            <div className="form-container" style={{ width: '450px', padding: '0', overflow: 'hidden' }}>
                ...
                {/* Conflict and Solved Buttons */}
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 1rem', background: '#e9ecef' }}>
                    <button 
                        onClick={() => setIsConflictModalOpen(true)}
                        style={{ padding: '0.5rem', background: '#e53935', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}
                    >
                        🚩 Report Conflict
                    </button>
                    ...
                    <button 
                        ...
                        style={{ padding: '0.5rem', background: isSolvedConfirmed ? '#adb5bd' : '#4CAF50', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}
                    >
