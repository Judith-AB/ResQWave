...
const ChatBox = ({ requestId, participantName, onClose }) => {
  ...
  const [isSolvedConfirmed, setIsSolvedConfirmed] = useState(
    request?.isResolvedByVolunteer || false
  );
  ...
  // Conflict check
  const isConflict =
    request?.status === "Conflict" || request?.status === "Atmost";
  ...
  const reportConflict = () => {
    if (
      !window.confirm(
        "Are you sure you want to report a conflict? Admin will be notified."
      )
    )
      return;
    ...
    socket.emit("raise_conflict", {
      ...
      reason: `${userRole} reported conflict.`,
    });
  ...
  const handleMarkSolved = () => {
    if (
      window.confirm("Are you sure the issue is fully resolved and completed?")
    ) {
  ...
  const handleSend = (e) => {
    ...
    addMessage(requestId, participantName, inputText.trim(), timestamp);
    setInputText("");
  };
      ...
      >
        ...
        >
          <h3 style={{ margin: 0, fontSize: "1.1rem" }}>
            💬 Request #{requestId} Chat: {userRole}
          </h3>
        ...
        >
          {messages.length === 0 ? (
            ...
            >
              Start communication for resolution.
            </p>
          ) : (
        ...
        >
          <button
            ...
            style={{
              padding: "0.5rem 1rem",
              background:
                isSolvedConfirmed || isConflict ? "#adb5bd" : "#4CAF50",
              color: "white",
              border: "none",
              borderRadius: "6px",
              cursor: isSolvedConfirmed || isConflict ? "not-allowed" : "pointer",
              fontWeight: "bold",
            }}
          >
            {isConflict
        ...
        <form
          onSubmit={handleSend}
          style={{ padding: "1rem", display: "flex", flexShrink: 0 }}
        >
          <input
            type="text"
            value={inputText}
