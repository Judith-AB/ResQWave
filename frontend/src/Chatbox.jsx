...
const ChatBox = ({ requestId, participantName, onClose }) => {
  ...
  const [isSolvedConfirmed, setIsSolvedConfirmed] = useState(
   
    (participantName === 'Admin' || participantName === 'Coordinator') ?
      (request?.isResolvedByVictim || false) : (request?.isResolvedByVolunteer || false)
  );

  const messages = getMessages(requestId);
  const messagesEndRef = useRef(null);
  const reqIdStr = String(requestId);
  const userRole = participantName === 'Admin' || participantName === 'Coordinator' ? 'Admin' : 'Volunteer';

  const isConflictOrSolved = request?.status === 'Conflict' || request?.status === 'Atmost' || request?.status === 'Completed';
  ...
  return (
    ...
    <div className="form-overlay" style={{
      ...
      <div className="form-container" style={{
        width: '450px', padding: '0', overflow: 'hidden', background: 'white',
        marginTop: '50px', borderRadius: '12px', boxShadow: '0 5px 20px rgba(0, 0, 0, 0.3)',
        ...
        <div style={{
          flexGrow: 1, overflowY: 'auto', padding: '1rem', background: '#f0f2f5'
        }}>
        ...
        <div style={{ padding: '0.5rem 1rem', borderTop: '1px solid #ccc', textAlign: 'right', background: '#e9ecef', flexShrink: 0 }}>
          <button
            ...
            disabled={isSolvedConfirmed || isConflictOrSolved}
            style={{
              padding: '0.5rem 1rem',
              background: (isSolvedConfirmed || isConflictOrSolved) ? '#adb5bd' : '#4CAF50',
              color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold'
            }}
          >
            {isConflictOrSolved ? '🚫 Admin Control' : isSolvedConfirmed ? '✅ Confirmed Solved' : 'Mark as Solved'}
          </button>
        </div>
